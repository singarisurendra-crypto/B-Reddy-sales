...
export default function Home() {
  ...
  const periodProfitSalesTotal=periodProfitSales.reduce((a,x)=>a+Number(x.total_amount||0),0);
  const periodProfitCost=periodProfitSales.reduce((total,sale)=>total+(sale.sale_items||[]).reduce((sum,line)=>{
  ...
  const periodExpenses=expenses.filter(e=>inPeriod(e.expense_date,dashboardProfitPeriod,dashboardPeriodEnd)).reduce((a,x)=>a+Number(x.amount||0),0);
  const periodGrossProfit=periodProfitSalesTotal-periodProfitCost;
